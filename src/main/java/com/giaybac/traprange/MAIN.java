...
import com.giaybac.traprange.entity.bean;
...
public class MAIN {
    ...
    private static void extractTables(String[] args) {
        try {
            ...
            List<bean> req_data = new ArrayList<>();
            List<Table> tables = extractor.extract();
           
            for(Table t: tables) {
            	String k = t.toString();
            	String [] k_x = k.split("\\n");
            	System.out.println(k_x[0]);
            	System.out.println(k_x[1]);
            	
            			for(int i=0;i<k_x.length-1;i++) {
            				 bean my_data = new bean();	
            			String[] n_x = k_x[i].split(";");
            			my_data.setDisabilityCatagory(n_x[1]);
            			my_data.setParticipants(n_x[2]);
            			my_data.setBallotsCompleted(n_x[3]);
            			my_data.setBallotsIncomplete(n_x[4]);
            			my_data.setAccuracy(n_x[5]);
            			my_data.setTimeToComplete(n_x[6]);
            				
            			req_data.add(my_data);     				
            	}
